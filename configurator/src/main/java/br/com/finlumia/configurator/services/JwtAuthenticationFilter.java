...
import br.com.finlumia.configurator.config.ApiPaths;
import br.com.finlumia.configurator.config.PublicApiProperties;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter implements ExternalApiAuthenticationFilter {
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) {
        ...
        boolean isProtected = requestUri.startsWith(ApiPaths.EXTERNAL_API_PREFIX + "/")
                || requestUri.startsWith(ApiPaths.CONFIG_API_PREFIX + "/");
        if (!isProtected) {
            return true;
        }
